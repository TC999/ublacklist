...
export default exportAsMessages({
  ...
  options_accessPermissionDescription: '同期機能を使って他のデバイスから SERPINFO の設定を変更した場合などに、アクセス許可の更新が必要になることがあります。',
  ...
  options_addSubscriptionDialog_altNameDescription: 'ダウンロードしたルールセットに名前が含まれないときに、代わりに使用される名前です。',
  ...
  options_blacklistHelper: '[マッチパターン](https://developer.mozilla.org/ja/docs/Mozilla/Add-ons/WebExtensions/Match_patterns)か[正規表現](https://developer.mozilla.org/ja/docs/Web/JavaScript/Guide/Regular_Expressions)が使用できます。',
  ...
  options_blockWholeSiteDescription: '例えば、ページ "https://a.b.example.uk.com/" をブロックすると、ルール "*://*.example.uk.com/*" が追加されます。',
  ...
  options_enableMatchingRules: '「このサイトをブロックする」ダイアログに「マッチするルール」を表示する',
  options_enableRulesetSubscriptionURL: 'ルールセット購読リンクを有効にする',
  options_enableRulesetSubscriptionURLButton: '有効にする',
  options_enableRulesetSubscriptionURLDescription: '$1 をこのページにリダイレクトし、ルールセットの購読を追加できるようにします。',
  ...
  options_enableSerpInfoSubscriptionURLDescription: '$1 をこのページにリダイレクトし、SERPINFO を追加できるようにします。',
  ...
  options_highlightDescription: '検索結果を色 N でハイライトするには、ルールの先頭に "@N" を追加します。',
  ...
  options_importBlacklistDialog_helper: 'Personal Blocklist からエクスポートしたドメインのリストを貼り付けて下さい。',
  ...
  options_otherSearchEnginesDescription: 'Google 以外の検索エンジンでもこの拡張機能を使用できるようにします。',
  ...
  options_subscriptionFeatureDescription: '購読を追加すると、指定した URL から定期的にルールセットがダウンロードされます。',
  ...
  options_syncFeatureDescription: 'クラウドを通して、複数の端末でブラックリストを同期することができます。',
  options_syncFeatureUpdated: '同期機能がアップデートされました。引き続き同期を使用するには、「同期を有効にする」ボタンを押して下さい。',
  ...
  options_turnOnSyncDialog_altFlowDescription: '認証中に $1 へのアクセス権限が求められることがありますが、個人情報がこのドメインに保存されることはありません。',
